...
async function main() {
  ...
  const { name, model, firstMessage, voice, transcriber, server, serverMessages } = buildAssistantPayload();
  const response = await fetch(`https://api.vapi.ai/assistant/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: { Authorization: `Bearer ${process.env.VAPI_API_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ name, model, firstMessage, voice, transcriber, server, serverMessages }),
